...
class DbHelper(context: Context) : SQLiteOpenHelper (context, "players", null , 1) {
    ...
    fun getLeaderboard(): ArrayList<Triple<String, String, String>> {
        val db = this.readableDatabase
        ...
        while (cursor.moveToNext()) {
            val player = Triple<String, String, String>(cursor.getString(0), cursor.getString(1), cursor.getString(2))
            list.add(player)
        }
